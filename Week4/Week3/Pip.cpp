...
void SwapStreams() {
	...
	//swap(strParam[PRIMARY_STREAM], strParam[SECONDARY_STREAM]);
...
void SetStreamDefault() {
	strParam[PRIMARY_STREAM] = defaultStreamSet[0];
	strParam[SECONDARY_STREAM] = defaultStreamSet[1];
	...
	g_nSubpictureID = SECONDARY_STREAM; 
...
void StartSwapAnimation(void) {
	strDelta[0].xPos = (strParam[1].xPos - strParam[0].xPos);
	strDelta[0].yPos = (strParam[1].yPos - strParam[0].yPos);
	strDelta[0].xSize = (strParam[1].xSize - strParam[0].xSize);
	strDelta[0].ySize = (strParam[1].ySize - strParam[0].ySize);
	
	strDelta[1].xPos = (strParam[0].xPos - strParam[1].xPos);
	strDelta[1].yPos = (strParam[0].yPos - strParam[1].yPos);
	strDelta[1].xSize = (strParam[0].xSize - strParam[1].xSize);
	strDelta[1].ySize = (strParam[0].ySize - strParam[1].ySize);

	strDelta[0].fAlapha = (strParam[1].fAlapha - strParam[0].fAlapha);
	strDelta[1].fAlapha = (strParam[0].fAlapha - strParam[1].fAlapha);
...
void AdjustStreamRect(int nStream, STRM_PARAM * pStream) {
	const int divisor = (UPDATE_TIMER / POSITION_TIMEOUT) * 2;
	if (!pStream)  return;

	strParam[nStream].xPos += pStream->xPos / divisor;
	strParam[nStream].yPos += pStream->yPos / divisor;
	strParam[nStream].xSize += pStream->xSize / divisor;
	strParam[nStream].ySize += pStream->ySize / divisor;
...
void AdjustAlpha(int nStream, STRM_PARAM * pStream) {
	...
	strParam[nStream].fAlapha += pStream->fAlapha;
...
HRESULT UpdatePinZOrder(int nStreamID, DWORD dw2Order) {
	...
	if (pMix)
		hr = pMix->SetZOrder(nStreamID, dw2Order);
	return hr;
}
...
LRESULT CALLBACK WndMainProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND:
		...
		case IDM_SWAP_ANIMATE: StartSwapAnimation();
			break;
		case IDM_SET_DEFAULT: SetStreamDefault();
			break;
		}
		break;
	case WM_CLOSE:
...
void OpenFiles() {
	...
	if (g_bFilesSelected) {
		...
		defaultStreamSet[0] = strParam[PRIMARY_STREAM];
		defaultStreamSet[1] = strParam[SECONDARY_STREAM];
		UpdatePinPos(0);
		UpdatePinPos(1);
		UpdatePinAlpha(0);
		UpdatePinAlpha(1);
